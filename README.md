...
The motivation is not theoretical. **TrapDoor** (May 2026, disclosed via the Anthropic Status disclosure pipeline) embedded hundreds of variation-selector codepoints in instructional markdown to smuggle hidden directives past human review while remaining fully visible to language models. **GlassWorm** (October 2025) used the same Unicode-tag-character technique against VS Code extensions, persisting payloads inside ostensibly-empty strings. The **Mini Shai-Hulud** worm (late 2025) compromised npm packages through self-replicating post-install scripts that targeted developer credentials. The common thread: the attack surface is the bytes an agent or extension reads before it executes — bytes humans rarely audit at the codepoint level.
